...
Feature: Issue #80: source file names not properly printed with python3
    ...
    | $ beehive -f pretty example/example.feature
    | Scenario: run a simple test         # example/example.feature:3
    |    Given we have beehive installed   # <string>:3
    |    When we implement a test         # <string>:7
    |    Then beehive will test it for us! # <string>:11
  ...
  Scenario: Show step locations
    When I run "beehive -c -f pretty --no-timings features/basic.feature"
    Then it should pass
