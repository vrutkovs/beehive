@issue
Feature: Issue #84: beehive.runner beehive does not reliably detected failed test runs

    | Behave does currently not reliably detected failed test runs and
    | therefore returns not sys.exit(1) at end of main().
    |
    | 1. beehive.runner:Runner.run_with_paths() returns failed==True
    |    if last feature was successful and test runner does not stop
    |    after first failing feature.
    |
    | 2. Issue #64: Same problem in beehive.model.Feature.run() with scenarios
  ...
  Scenario: First feature fails, second feature passes
    When I run "beehive -f plain features/failing.feature features/passing.feature"
  ...
  Scenario: First feature passes, second feature fails
    When I run "beehive -f plain features/passing.feature features/failing.feature"
    Then it should fail with:
        """
        1 feature passed, 1 failed, 0 skipped
        1 scenario passed, 1 failed, 0 skipped
  ...
  Scenario: First feature passes, second fails, last passes
    When I run "beehive -f plain features/passing.feature features/failing.feature features/passing.feature"
    Then it should fail with:
        """
