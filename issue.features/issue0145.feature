...
Feature: Issue #145: before_feature/after_feature should not be skipped
  ...
  Scenario: Select only @scenario tag
    When I run "beehive -f plain -T --tags=@scenario features/issue0145_example.feature"
    Then it should pass with:
        ...
        """
    And the beehive hook "before_feature" was called
    And the beehive hook "after_feature" was called

  Scenario: Select @feature tag and @scenario tag (logical-and, fails if not fixed)
    When I run "beehive -f plain -T --tags=@feature --tags=@scenario features/issue0145_example.feature"
    Then it should pass with:
        """
        1 feature passed, 0 failed, 0 skipped
        1 scenario passed, 0 failed, 0 skipped
