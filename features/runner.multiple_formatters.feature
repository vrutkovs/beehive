Feature: Multiple Formatter with different outputs
    ...
    Scenario: One formatter, no outfile (use stdout instead)
      Given a file named "beehive.ini" does not exist
    ...
    Scenario: One formatter, one outfile
      Given a file named "beehive.ini" does not exist
      When I run "beehive -f plain --outfile=output/plain.out -T features/"
      Then it should pass with:
        """
        2 features passed, 0 failed, 0 skipped
        2 scenarios passed, 0 failed, 0 skipped
    ...
    Scenario: Two formatter, one outfile
      Given a file named "beehive.ini" does not exist
      When I run "beehive -f plain -o output/plain.out -f progress -T features/"
      Then it should pass with:
        """
        2 features passed, 0 failed, 0 skipped
        2 scenarios passed, 0 failed, 0 skipped
    ...
    Scenario: More outfiles than formatter should fail with CONFIG-ERROR
      Given a file named "beehive.ini" does not exist
      When I run "beehive -f plain -o plain.output -o xxx.output features/"
    ...
    @with_configfile
    Scenario: Use default formatter and outfile from beehive configuration file
    ...
    @with_configfile
    Scenario: Use default formatter and another without outfile from beehive configuration file
      Given a file named "beehive.ini" with:
        """
        [beehive]
    ...
    @with_configfile
    Scenario: Command-line formatter/outfile extend beehive configuration file args
      Given a file named "beehive.ini" with:
        """
        [beehive]
        show_timings = false
        ...
        """
      And I remove the directory "output"
      When I run "beehive -c -f pretty -o output/pretty.out -f progress -o output/progress.out features/"
      Then it should pass
    ...
    Scenario: Combination of formatter from configfile and command-line cannot cause outfile offsets
      Given a file named "beehive.ini" with:
        ...
        """
      And I remove the directory "output"
      When I run "beehive -f progress -o output/progress.out features/"
      Then it should pass
      And the command output should not contain:
        """
        Feature: Alice
