...
Feature: Capture log output
  ...
  |  * log_capture mode can be defined in beehive configuration file
  ...
  Scenario: Feature Setup
    ...
    And a file named "features/steps/use_beehive4cmd_steps.py" with:
        """
    ...
    Scenario: Captured log is suppressed if scenario passes
        When I run "beehive -f plain -T --logcapture features/example.log_and_pass.feature"
    ...
    Scenario: Captured log is shown up to first failure if scenario fails
        When I run "beehive -f plain -T --logcapture features/example.log_and_fail.feature"
    ...
    Scenario: Captured log is shown up to first failure if log_record.level is not too low
      ...
      Ensure that only log-records are shown that exceed the logging-level.
        ...
        When I run "beehive -f plain --logcapture --logging-level=ERROR features/example.log_and_fail.feature"
        Then it should fail
        And the command output should contain the following log records:
            | category | level   | message |
            | root     |  FATAL  | Hello Alice  |
    ...
    Scenario: Logcapture mode is enabled per default
      When I run "beehive -f plain features/example.log_and_fail.feature"
      Then it should fail
      And the command output should contain "Captured logging:"


    Scenario: Logcapture mode can be enabled on command-line
      When I run "beehive -f plain --logcapture features/example.log_and_fail.feature"
      Then it should fail
      And the command output should contain "Captured logging:"


    Scenario: Logcapture mode can be disabled on command-line
      When I run "beehive -f plain --no-logcapture features/example.log_and_fail.feature"
      Then it should fail
