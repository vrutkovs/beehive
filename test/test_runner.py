...
class TestContext(unittest.TestCase):
    ...
    def test_user_mode_shall_restore_beehive_mode(self):
        # -- CASE: No exception is raised.
        with self.context.user_mode():
            eq_(self.context._mode, runner.Context.USER)
            self.context.thing = 'stuff'
        eq_(self.context._mode, runner.Context.BEEHIVE)

    def test_user_mode_shall_restore_beehive_mode_if_assert_fails(self):
        try:
            with self.context.user_mode():
                eq_(self.context._mode, runner.Context.USER)
                assert False, "XFAIL"
    ...
    def test_user_mode_shall_restore_beehive_mode_if_exception_is_raised(self):
        try:
            with self.context.user_mode():
                eq_(self.context._mode, runner.Context.USER)
    ...
    def test_beehive_masking_user_attribute_causes_warning(self):
        warns = []
        ...
        assert isinstance(warning, runner.ContextMaskWarning), 'warning is not a ContextMaskWarning'
        info = warning.args[0]
        assert info.startswith('beehive runner'), "%r doesn't start with 'beehive runner'" % info
        assert "'thing'" in info, '%r not in %r' % ("'thing'", info)
...
class TestContext_ExecuteSteps(unittest.TestCase):
    """
    Test the beehive.runner.Context.execute_steps() functionality.
    """
    step_registry = None
...
class TestRunner(object):
    def test_load_hooks_execfiles_hook_file(self):
        with patch('beehive.runner.exec_file') as ef:
    ...
    @patch('beehive.runner.LoggingCapture')
...
class TestRunWithPaths(unittest.TestCase):
    def setUp(self):
        ...
        self.formatter_class = patch('beehive.formatter.pretty.PrettyFormatter')
    ...
    @patch('beehive.parser.parse_file')
