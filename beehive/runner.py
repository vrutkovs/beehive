    ...
    *beehive* may overwrite the value you set. These names are:
    ...
    def _emit_warning(self, attr, params):
        ...
        if self._mode is self.BEEHIVE and self._origin[attr] is not self.BEEHIVE:
            msg = "beehive runner is masking context attribute '%(attr)s' " \
        ...
        if new_base_dir == root_dir:
            if self.config.verbose:
                if not self.config.paths:
                    print('ERROR: Could not find "%s" directory. Please ' % self.config.steps_dir +
                          'specify where to find your features.')
                else:
                    print('ERROR: Could not find "%s" directory in your ' % self.config.steps_dir +
